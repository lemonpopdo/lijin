...
const data = ref<Api.SearchOutput | null>(null)
...
watch(keyword, async (val) => {
  ...
  loading.value = true
  try {
    data.value = await apiSearchGet({ keyword: val })
  }
  finally {
    loading.value = false
  }
...
const onMoreClick = (type?: string) => {
  switch (type) {
    case 'friend':
      uni.navigateTo({
        url: `/pages/search/more/friend?keyword=${encodeURIComponent(keyword.value)}`,
      })
      break
    case 'book-item':
      uni.navigateTo({
        url: `/pages/search/more/bookItem?keyword=${encodeURIComponent(keyword.value)}`,
      })
      break
    case 'gift':
      uni.navigateTo({
        url: `/pages/search/more/gift?keyword=${encodeURIComponent(keyword.value)}`,
      })
      break
  }
}
      ...
      <div v-else class="pb-4">
        ...
        <div v-show="data?.friends?.length" class="mt-2 bg-white p-3">
          <div class="mb-2 flex items-center justify-between">
            ...
            <div v-show="(data?.friends?.length ?? 0) > 4" class="flex items-center text-xs text-gray" @click="onMoreClick('friend')">
              更多
        ...
        <div v-show="data?.bookItems?.length" class="mt-2 bg-white p-3">
          <div class="mb-2 flex items-center justify-between">
            ...
            <div v-show="(data?.bookItems?.length ?? 0) > 4" class="flex items-center text-xs text-gray" @click="onMoreClick('book-item')">
              更多
              <i class="i-hugeicons-arrow-right-01 ml-0.5" />
            </div>
          </div>
        ...
        <div v-show="data?.gifts?.length" class="mt-2 bg-white p-3">
          <div class="mb-2 flex items-center justify-between">
            ...
            <div v-show="(data?.gifts?.length ?? 0) > 4" class="flex items-center text-xs text-gray" @click="onMoreClick('gift')">
              更多
              <i class="i-hugeicons-arrow-right-01 ml-0.5" />
            </div>
          </div>
